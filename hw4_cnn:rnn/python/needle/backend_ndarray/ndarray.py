...
class NDArray:
    ...
    def reshape(self, new_shape):
        ...
        # check that old shape has same number of elements as new shape and array is compact
        if prod(self.shape) != prod(new_shape) or not self.is_compact():
          raise ValueError
    
        # .make() computes the compact stride when stride isnt an arg; linear view of data maintained
    ...
    def permute(self, new_axes):
        ...
        # check if the new_axes typle is valid
        if not isinstance(new_axes, tuple):
          raise TypeError

        if min(new_axes) != 0 or max(new_axes) != (self.ndim -1) or len(set(new_axes)) != self.ndim:
          raise ValueError
        
        # extract new shape from the permuted axis; swap strides accordingly
        return NDArray.make(shape=tuple([self._shape[axis] for axis in new_axes]),
                            strides=tuple([self._strides[axis] for axis in new_axes]),
    ...
    def broadcast_to(self, new_shape):
        ...
        error_msg = "Need all dims to match unless init shape has a 1 value in that dim"
        assert all([x == y or x == 1 for x,y in zip(self._shape, new_shape)]), error_msg

        # new array has shape new_shape and has strides equal to self for matching areas, stride = 0 for when self_shape = 1
        broadcasted_strides = []
        for i, (x,y) in enumerate(zip(self._shape, new_shape)):
          if x == 1:
              broadcasted_strides.append(0)
          elif x == y:
              broadcasted_strides.append(self._strides[i])
          else:
              print(f"Error: Should Never Get Here")
              raise RuntimeError
        return NDArray.make(shape=new_shape, 
                            strides=tuple(broadcasted_strides), 
                            device=self._device,
                            handle=self._handle)
    ...
    def __getitem__(self, idxs):
        ...
        # loop over all the slices. For each slice:
        # get new shape by getting the difference between slice.start and slice.stop
        # get offset by multiplying start by stride of this dim
        # stride stays the same as before? what if the step is greater then 1?
        ...
        for i, slice_ in enumerate(idxs):
          ...
          new_shape.append((stop-start + step -1)//step) # will be 1 if we slice in a single index ; round up
    ...
    def reduce_view_out(self, axis, keepdims=False):
        ...
        if axis is None:
            ...
            # out = NDArray.make((1,), device=self.device)1
            # SCARY SCARY SCARY!!!!!!!!!!!
    ...
    def flip(self, axes):
        ...
        # compute running product; by convention, right most element = 1
        prod_array = [1]*len(old_shape)
        for i in range(len(old_shape)-2,-1,-1):
            prod_array[i] = prod_array[i+1]*old_shape[i+1]
        
        # compute offset
        for axis in axes:
            new_stride[axis] = -new_stride[axis] # ensure that the stride of flipped axis is negative
            new_offset += prod_array[axis]*(old_shape[axis]-1) 

        # make new array, compact it, return
        return NDArray.make(self._shape, 
                            tuple(new_stride), 
                            device=self._device, 
                            handle=self._handle, 
    ...
    def pad(self, axes):
        ...
        """
        ### BEGIN YOUR SOLUTION
        # ensure that axes is same length as dim of old array
        if len(axes) != self.ndim:
            raise ValueError(f"Need to specify padding for each dim of the array; array has {self.ndim} dims but axes is only specifed {len(axes)}")
        
        # construct new shape and slices
        old_shape = self._shape # 5 x 5 x 5
        new_shape = []

        # make list of slices we will use to get a slice of the new, all zeros array
        slices = []
        for i, (pad1, pad2) in enumerate(axes):
            new_shape.append(old_shape[i] + pad1 + pad2)
            slices.append(slice(pad1, old_shape[i] + pad1))
        
        # construct all zeros array of the new shape on the same device as the current array
        out = full(tuple(new_shape), fill_value=0, device=self._device)
        
        # slice into it with the shape of the old, prepadded array
