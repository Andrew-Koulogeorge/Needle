...
__global__ void CompactKernel(const scalar_t* a, scalar_t* out, size_t size, CudaVec shape,
   ...
   */
  size_t gid = blockIdx.x * blockDim.x + threadIdx.x; // replaces cnt loop var in cpp version
  size_t cpy_gid = gid;
  if(gid < size){ // ensure that id computed from block and thread index are within array bounds;
                  // as there could be more threads deployed then entries to operate on
    size_t idx = offset; // index into a starts at offset
    size_t stride_idx = strides.size - 1; // each stride value used to index into array
    for(int32_t shape_idx = shape.size - 1; shape_idx >=0; shape_idx--){
      // increment idx by the strides times the mod of this shape
      size_t shp = (size_t)shape.data[shape_idx];
      idx += strides.data[stride_idx] * (gid % shp);
      stride_idx--;
      gid /= shp;
    }
    out[cpy_gid] = a[idx];
...
__global__ void MatmulKernelv1(const float *a, const float *b, float *out, uint32_t M, uint32_t N,
            uint32_t P)
{
  // compute thread index 
  uint32_t gid = blockIdx.x*blockDim.x + threadIdx.x;
  if(gid < M*P){ // within the grid
    // extract an i,j index from the gid and then compute the dot product
    uint32_t i = gid / P;
    uint32_t j = gid % P;
    scalar_t val = 0;
...
void Matmul(const CudaArray& a, const CudaArray& b, CudaArray* out, uint32_t M, uint32_t N,
  ...
  MatmulKernelv1<<<dim.grid, dim.block>>>(a.ptr, b.ptr, out->ptr,M,N,P);
}
...
__global__ void ReduceMaxKernel(const scalar_t *a, scalar_t *out, size_t reduce_size, size_t output_size){
  // compute the index specific to this thread
  size_t gid = blockIdx.x* blockDim.x + threadIdx.x;
  if(gid < output_size)
  {  
    scalar_t max_so_far = a[reduce_size*gid];
    for(size_t j = 0; j < reduce_size; j++)
      max_so_far = max(max_so_far, a[reduce_size*gid + j]);     
    out[gid] = max_so_far;
  }
}
...
void ReduceMax(const CudaArray& a, CudaArray* out, size_t reduce_size) {
  ...
  CudaDims dim = CudaOneDim(output_size); // each thread will perform a reduction across the reduced axis
  ReduceMaxKernel<<<dim.grid, dim.block>>>(a.ptr, out->ptr, reduce_size, output_size);
}

__global__ void ReduceSumKernel(const scalar_t *a, scalar_t *out, size_t reduce_size, size_t output_size){
  // compute the index specific to this thread
  size_t gid = blockIdx.x* blockDim.x + threadIdx.x;
  if(gid < output_size)
  {
    ...
    for(size_t j = 0; j < reduce_size; j++)
      running_sum += a[reduce_size*gid + j];
    out[gid] = running_sum;
  }
}
...
void ReduceSum(const CudaArray& a, CudaArray* out, size_t reduce_size) {
  ...
  size_t output_size = out->size;
  CudaDims dim = CudaOneDim(output_size); // each thread will perform a reduction across the reduced axis
  ReduceSumKernel<<<dim.grid, dim.block>>>(a.ptr, out->ptr, reduce_size, output_size);
}
